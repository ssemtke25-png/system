...
# ==========================================
# [2. 데이터 파싱 함수 (오류 완벽 제거판)]
# ==========================================
def clean_reg_text(text):
    """규정 원본에 섞여있는 질의회신, 판례 꼬리표를 싹둑 잘라내는 마법의 필터"""
    keywords = ["【질의회신", "[질의회신", "【참고판례", "[참고판례", "【판례", "[판례"]
    for kw in keywords:
        idx = text.find(kw)
        if idx != -1:
            text = text[:idx] # 해당 키워드가 나오면 그 앞까지만 남기고 자름
    return text.strip()
                ...
                if html_content:
                    ...
                    for tr in soup.select("tr"):
                        ...
                        if tds and (jo_span := tr.select_one("span.bl")):
                            ...
                            # 여기서 불필요한 꼬리표를 제거!
                            reg_list.append({"조문": jo_span.get_text(strip=True), "내용": clean_reg_text(raw_content)})
                    ...
                    if not reg_list:
                        ...
                        for line in text_lines:
                            ...
                            if re.match(r'^제\s*\d+\s*조', line) or re.match(r'^\[별표', line):
                                if current_content: 
                                    reg_list.append({"조문": current_jo, "내용": clean_reg_text("\n".join(current_content))})
...
# 🚨 Streamlit 렌더링 에러(NotFoundError)를 완벽 차단하는 보호 캡슐!
def render_safe_html(text, kw=""):
    safe = str(text).replace("<", "&lt;").replace(">", "&gt;") # 꺾쇠 무력화
    safe = safe.replace("\n", "<br>") # 줄바꿈 유지
    if kw:
        safe_kw = str(kw).replace("<", "&lt;").replace(">", "&gt;")
        if safe_kw:
            safe = safe.replace(safe_kw, f"<mark style='background-color: yellow;'>{safe_kw}</mark>")
    # 전체를 <div>로 감싸서 React DOM 에러 원천 차단
...
st.markdown("---")
st.caption("v6.0 UX Pro - 규정 데이터 자동 정제 & 렌더링 에러(NotFoundError) 원천 차단 완결판")
