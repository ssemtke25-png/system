...
def ask_ai(question, materials):
    ...
    system_instruction = (
        ...
        "2. 띄어쓰기가 안되어 있어도 관련 내용을 찾아 검색하세요. 예) 경미한변경 -> 경미한 변경.\n"
        "3. 자료에 없는 사실·수치·조항 번호·날짜·금액은 절대로 추가하거나 지어내지 마세요. "
        "만약 자료에서 확인되지 않으면 '제공된 자료에서는 확인되지 않습니다'라고 명확히 밝히세요.\n"
        "4. 답변에 사용한 근거는 반드시 [자료1], [자료2] 형태로 어느 자료에서 나왔는지 표시하세요.\n"
        "\n"
        ...
        "- 답변이 너무 길어질 것 같으면 핵심 위주로 정리하되, 반드시 문장과 설명을 끝까지 완결지어 마무리하세요. 중간에 문장이 끊기지 않도록 하세요."
    )
