...
import base64
import gspread
import html  # <--- 에러를 잡기 위한 마법의 모듈 추가!
...
# 🚨 에러 해결의 핵심! 텍스트를 무조건 안전하게 변환하는 마법 함수
def safe_markdown(text, kw=""):
    safe_text = html.escape(text).replace("\n", "<br>")
    if kw:
        safe_kw = html.escape(kw)
        safe_text = safe_text.replace(safe_kw, f"<mark style='background-color: yellow;'>{safe_kw}</mark>")
    return safe_text
...
if mode in ["📑 질의회신", "🧑‍⚖️ 판례"]:
    ...
    for idx, row in res.iterrows(): 
        ...
        with st.expander(f"{icon} {row['제목']}"):
            st.markdown(safe_markdown(row['내용'], keyword), unsafe_allow_html=True)
        ...
        for item in law_db:
            ...
            if match:
                ...
                with st.expander(f"⚖️ [법령] {item['조문']}"):
                    st.markdown(f"**📜 [법률]**<br>{safe_markdown(item['법률'], keyword)}", unsafe_allow_html=True)
                    st.markdown("---")
                    st.markdown(f"**⚙️ [시행령]**<br>{safe_markdown(item['시행령'], keyword)}", unsafe_allow_html=True)
                    if item['시행규칙'].strip():
                        st.markdown("---")
                        st.markdown(f"**📋 [시행규칙]**<br>{safe_markdown(item['시행규칙'], keyword)}", unsafe_allow_html=True)
        st.caption(f"총 {count}건의 조문이 검색되었습니다.")
    else:
        st.caption(f"총 {len(law_db)}개의 전체 조문입니다.")
        for item in law_db:
            with st.expander(f"⚖️ [법령] {item['조문']}"):
                st.markdown(f"**📜 [법률]**<br>{safe_markdown(item['법률'])}", unsafe_allow_html=True)
                st.markdown("---")
                st.markdown(f"**⚙️ [시행령]**<br>{safe_markdown(item['시행령'])}", unsafe_allow_html=True)
                if item['시행규칙'].strip():
                    st.markdown("---")
                    st.markdown(f"**📋 [시행규칙]**<br>{safe_markdown(item['시행규칙'])}", unsafe_allow_html=True)
            ...
            for item in reg_data:
                ...
                if match:
                    ...
                    with st.expander(f"📖 [{display_name}] {item['조문']}"):
                        st.markdown(safe_markdown(item['내용'], keyword), unsafe_allow_html=True)
...
st.markdown("---")
st.caption("v5.9 UX Pro - 꺾쇠(<>) 에러 완벽 해결 및 공지사항 통합 버전")
