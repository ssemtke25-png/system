...
수정사항 (v3 신규):
- 한 파일에 현황보고/미수납조서 시트가 여러 개인 경우 모두 처리
  예) 01.포항.xlsx → '현황보고(포항시 남구)', '현황보고(포항시 북구)' 시트 둘 다 취합
  → find_sheet(첫 번째만) → find_all_sheets(전체 목록)으로 변경

v2 수정사항 유지:
- has_protective_color theme색 오판 수정
- SHEET1_BASE_COLS 열3~16
- accumulate_sheet1 offset 제거
- 시트2 서식 완전 복사
- 이행강제금 시트 없는 파일 정상 처리
- docProps/custom.xml name=None 오류 수정 (한글오피스 파일)
"""
import io
import re
import zipfile
...
def find_all_sheets(wb, candidates):
    """
    [신규] 매칭되는 시트 전체 목록 반환
    한 파일에 현황보고(포항시 남구), 현황보고(포항시 북구) 처럼
    여러 시트가 있을 때 모두 처리하기 위함
    """
    result = []
    # 정확히 일치하는 것 먼저
    for name in wb.sheetnames:
        for c in candidates:
            if name == c or c.replace(' ', '') == name.replace(' ', ''):
                if wb[name] not in result:
                    result.append(wb[name])
    # 부분 일치 (시군명 포함된 시트명)
    for name in wb.sheetnames:
        ws = wb[name]
        if ws in result:
...
def fix_custom_props(zin, zout):
    """docProps/custom.xml name=None 항목 제거 (한글오피스 저장 파일 대응)"""
...
def aggregate(template_bytes, region_files):
    ...
    for fname, _ in sorted_files:
        if fname not in loaded:
            continue
        # [핵심 수정] 현황보고 시트 전체 목록 가져와서 모두 누적
        src_ws1_list = find_all_sheets(loaded[fname], SHEET1_CANDIDATES)
        if not src_ws1_list:
            warnings.append({"유형": "시트 없음", "파일": fname, "설명": "현황보고 시트를 찾지 못함"})
            continue
        for src_ws1 in src_ws1_list:
            n = accumulate_sheet1(base_ws1, src_ws1, input_cells, warnings, fname)
            log.append({"파일": fname, "시트": src_ws1.title, "누적셀수": n})
    ...
    for fname, _ in sorted_files:
        if fname not in loaded:
            continue
        # [핵심 수정] 미수납조서 시트 전체 목록 가져와서 모두 이어붙임
        src_ws2_list = find_all_sheets(loaded[fname], SHEET2_CANDIDATES)
        if not src_ws2_list:
            log.append({"파일": fname, "시트": "미수납조서", "추가행수": 0,
                        "비고": "미수납조서 시트 없음 (이행강제금 없는 파일)"})
            continue
        for src_ws2 in src_ws2_list:
            next_row, n = append_sheet2(base_ws2, src_ws2, next_row, warnings, fname)
            log.append({"파일": fname, "시트": src_ws2.title, "추가행수": n})
...
def render():
    ...
    st.info(
        "📌 한 파일에 여러 시군 시트(예: 포항남구+포항북구)가 있으면 모두 취합합니다.\n\n"
