...
2026-08 수정2: 위성=ESRI 유지 + 이미지 하단 출처(attribution) 자동 표기,
               VWorld 운영키 확보 시 _tile_sources 함수만 교체하면 전환 완료.
"""
...
# ── 7. 행사장 약도 ────────────────────────────────────────────────────
#  2026-08 수정2: 위성=ESRI 유지 + 이미지 하단 출처(attribution) 자동 표기.
#  ▶ VWorld 운영키 확보 시 _tile_sources() 함수의 satellite 분기만 교체.
# ══════════════════════════════════════════════════════════════════════
...
# ── 지도 소스 정의 ────────────────────────────────────────────────────
# ★ VWorld 운영키를 받으면 아래 satellite 분기만 교체하면 됩니다.
#   (st.secrets["VWORLD_KEY"] 추가 후 URL과 attribution 문구 변경)
#
#   VWORLD 예시:
#     key = st.secrets["VWORLD_KEY"]
#     return (
#         f"https://api.vworld.kr/req/wmts/1.0.0/{key}/Satellite/{{z}}/{{y}}/{{x}}.jpeg",
#         f"https://api.vworld.kr/req/wmts/1.0.0/{key}/Hybrid/{{z}}/{{y}}/{{x}}.png",
#         "출처: 국토교통부 브이월드(VWorld)",
#     )
#
...
def _osm_static_via_tiles(lat, lng, slider_zoom, width, height, maptype="satellite"):
    ...
    가장 안정적인 최종 폴백. 이미지 하단에 출처(attribution) 자동 표기.
    실패 시 None 반환.
...
def render_map():
    ...
    if st.session_state.get("map_img"):
        ...
        st.caption("💡 아래 버튼으로 저장한 PNG를 계획서(HWP/워드)에 그대로 붙여넣으세요. "
                   "화면에 보이는 이미지가 그대로 저장됩니다. (출처가 이미지 하단에 포함됩니다)")
