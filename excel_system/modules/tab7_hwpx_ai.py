...
마크다운 헤딩(#, ##)으로 구조화하고 아래 항목을 포함하세요:
# 행사 개요, ## 추진 배경 및 목적, ## 행사 내용, ## 소요 예산, ## 기대 효과
공문서 형식으로 간결하고 명확하게 작성하세요.""",
    },
    "국장인사말": {
        "prompt": """다음 행사 계획서 요약을 바탕으로 국장급 인사말을 작성하세요.
- 격식 있고 품위 있는 문체
- 참석자에 대한 감사 인사
- 행사의 의미와 중요성 강조
- 기관의 노력과 앞으로의 방향 제시
...
마크다운 없이 순수 텍스트로 작성하세요.""",
    ...
    "과장인사말": {
        "prompt": """다음 행사 계획서 요약을 바탕으로 과장급 인사말을 작성하세요.
- 친근하고 따뜻한 문체
- 참석자와의 공감대 형성
- 행사 목적 간략 설명
- 300~500자 분량
마크다운 없이 순수 텍스트로 작성하세요.""",
    ...
    "보도자료": {
        "prompt": """다음 행사 계획서 요약을 바탕으로 언론 배포용 보도자료를 작성하세요.
- 제목: 임팩트 있는 헤드라인 (# 헤딩)
- 부제: 핵심 내용 요약 (## 헤딩)
- 본문: 육하원칙(누가/언제/어디서/무엇을/왜/어떻게)에 따라 작성
- 인용구: 담당자 코멘트 포함 (따옴표 사용)
- 문의처: 담당부서/담당자 정보
공식 보도자료 형식으로 작성하세요.""",
    },
}
...
def render_mc():
    ...
    if st.button("✍️ 사회자 멘트 생성", key="mc_gen"):
        prompt = f"""당신은 공공기관 행사 전문 사회자입니다.
아래 행사 정보와 순서에 맞는 사회자 멘트를 작성하세요.
...
[작성 규칙]
- 각 순서는 ## 헤딩으로 구분
- 멘트는 실제 현장에서 바로 읽을 수 있는 완성된 문장
- 각 순서별 100~200자 내외
- 자연스러운 전환 문구 포함
- 참석자와의 호흡을 고려한 자연스러운 흐름 유지"""
...
def render_banner():
    ...
    if st.button("✍️ 현수막 문구 생성", key="banner_gen"):
        prompt = f"""당신은 공공기관 행사 홍보 전문가입니다.
다음 행사 계획서를 바탕으로 현수막 문구 시안 {n}개를 작성하세요.

[스타일]: {style}
[행사 계획서 요약]: {summary_str}

[작성 규칙]
- 각 시안은 15~25자 이내로 간결하게
- 행사 목적과 핵심 메시지가 담길 것
- 번호. 문구 형식으로 출력
- 각 문구 아래 한 줄 설명 추가 (어떤 상황에 어울리는지)
- 공식형: 격식있고 전문적인 어조
- 친근형: 따뜻하고 공감가는 어조
- 역동형: 강렬하고 임팩트 있는 어조"""
...
def render_result_report():
    ...
    if st.button("✍️ 결과보고서 초안 생성", key="result_gen"):
        prompt = f"""당신은 공공기관 행사 담당자입니다.
아래 행사 계획서 요약과 결과 정보를 바탕으로 행사 결과보고서 초안을 작성하세요.
...
# 5. PPT 자동생성
# ─────────────────────────────────────────────
PPT_PROMPT_BASE = """당신은 공공기관 행사 PPT를 전문으로 만드는 전문가입니다.
...
예시:
slide_num, layout(title/content/section/table/closing), title, subtitle(표지·클로징만),
bullets(content용 리스트), headers(table용 컬럼명 리스트), rows(table용 2차원 리스트)"""
