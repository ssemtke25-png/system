...
- 수치와 사실 위주로 간결하게 작성
- 제목만 달랑 있는 슬라이드 금지 — 반드시 본문 내용(body)을 채워라
- 담당자가 별도 편집 없이 바로 발표할 수 있는 수준으로 작성

[사용 가능한 레이아웃 7종]

1. "title" — 표지 슬라이드
   - title: 행사명
   - subtitle: "일시 | 장소 | 주최기관" 형식

2. "closing" — 마무리 슬라이드
   - title: "감사합니다" 또는 행사 슬로건
   - subtitle: 담당부서 및 연락처

3. "section" — 챕터 구분
   - title: 섹션명만

4. "content" — 제목 + 불릿 본문 (가장 일반적)
   - title: 슬라이드 제목
   - body: 슬라이드 전체 내용을 2~3문장으로 요약한 본문 (반드시 작성)
   - bullets: 핵심 항목 3~6개 (각 20자 이내 간결하게)

5. "two_column" — 좌우 비교 (AS-IS vs TO-BE, 계획 vs 실적 등)
   - title: 슬라이드 제목
   - body: 비교 배경 한 문장
   - left_title: 왼쪽 컬럼 제목
   - left_bullets: 왼쪽 내용 3~4개
   - right_title: 오른쪽 컬럼 제목
   - right_bullets: 오른쪽 내용 3~4개

6. "highlight" — 핵심 숫자/성과 강조
   - title: 슬라이드 제목
   - body: 배경 설명 한 문장
   - stat_number: 강조할 숫자 (예: "150명", "3회", "98%")
   - stat_label: 숫자 설명 (예: "참석 인원", "누적 개최", "예산 집행률")
   - bullets: 부연 설명 2~4개

7. "table" — 표 형식 (일정표, 역할분담표 등)
   - title: 슬라이드 제목
   - body: 표 설명 한 문장
   - headers: 컬럼명 리스트
   - rows: 2차원 데이터 배열
...
[필수 슬라이드 구성 순서]
표지(title) → 목차(content, bullets에 01~형식) → 행사 개요(highlight, 참석인원 강조) → 추진 배경(two_column, 현황 vs 목표) → 주요 프로그램(table) → 세부 내용(content, 계획서 내용 반영) → 기대 효과(content) → 클로징(closing)

[출력 형식 — 반드시 순수 JSON 배열만, 마크다운 코드블록 없이]
모든 슬라이드에 body 필드를 반드시 포함하라 (title/closing/section 제외)."""
...
def _build_pptx(slides_data: list, summary: dict, theme: str = "네이비 (공공기관 정장)") -> bytes | None:
    """다크 배경 고품질 PPT - 7종 레이아웃 + body 본문 지원"""
    try:
        ...
        def rgb(key): return RGBColor(*pal[key])
        def WHITE():  return RGBColor(0xFF, 0xFF, 0xFF)
        ...
        def txt(slide, text, x, y, w, h, size, bold=False,
                color_key=None, rgb_val=None, align=PP_ALIGN.LEFT,
            ...
            r.font.color.rgb = rgb(color_key) if color_key else RGBColor(*(rgb_val or (255,255,255)))
            return tb

        def add_bullets(slide, bullets, x, y, w, h, size=17, accent_dot=True):
            """불릿 리스트 추가"""
            tb = slide.shapes.add_textbox(x, y, w, h)
            tf = tb.text_frame
            tf.word_wrap = True
            for i, b in enumerate(bullets):
                ...
                p2.space_after  = Pt(3)
                r2 = p2.add_run()
                r2.text = f"  ▸  {b}" if accent_dot else f"  • {b}"
                r2.font.size = Pt(size)
                r2.font.name = MALGUN_GOTHIC
                r2.font.color.rgb = rgb("text")

        def add_body(slide, body_text, x, y, w, h, size=14):
            """본문 설명 텍스트 추가 (subtext 색상)"""
            if not body_text:
                return
            tb = slide.shapes.add_textbox(x, y, w, h)
            tf = tb.text_frame
            ...
            p = tf.paragraphs[0]
            r = p.add_run()
            r.text = str(body_text)
            r.font.size = Pt(size)
            r.font.name = MALGUN_GOTHIC
            r.font.color.rgb = rgb("subtext")

        def header_panel(slide, title_t, body_text="", h=Inches(1.5)):
            """공통 상단 헤더 패널"""
            rect(slide, 0, 0, W, h, "bg2")
            rect(slide, 0, 0, Inches(0.12), H, "accent")
            rect(slide, Inches(0.12), h - Inches(0.05), W, Inches(0.05), "accent2")
            txt(slide, title_t,
                Inches(0.3), Inches(0.15), Inches(12.5), Inches(0.75),
                size=26, bold=True, color_key="text")
            if body_text:
                add_body(slide, body_text,
                         Inches(0.3), Inches(0.9), Inches(12.5), Inches(0.5),
                         size=13)
        ...
        for idx, si in enumerate(slides_data):
            ...
            if layout == "title":
                ...
                if org:
                    txt(slide, org, Inches(0.5), Inches(0.35), Inches(8), Inches(0.5),
            ...
            # ══ section ════════════════════════════════
                ...
                txt(slide, f"{snum-1:02d}",
                    Inches(0.4), Inches(0.8), Inches(7.5), Inches(2.5),
            ...
            # ══ highlight (핵심 숫자 강조) ═════════════
                ...
                stat_number = si.get("stat_number", "")
                stat_label  = si.get("stat_label", "")

                header_panel(slide, title_t, body)

                # 좌측: 큰 숫자
                rect(slide, Inches(0.3), Inches(1.65), Inches(4.5), Inches(5.2), "card")
                rect(slide, Inches(0.3), Inches(1.65), Inches(0.1), Inches(5.2), "accent2")
                txt(slide, stat_number,
                    Inches(0.5), Inches(2.3), Inches(4.1), Inches(2.5),
                    size=72, bold=True, color_key="accent2", align=PP_ALIGN.CENTER)
                txt(slide, stat_label,
                    Inches(0.5), Inches(5.0), Inches(4.1), Inches(0.6),
                    size=16, color_key="subtext", align=PP_ALIGN.CENTER)

                # 우측: 불릿
                if bullets:
                    add_bullets(slide, bullets,
                                Inches(5.1), Inches(1.7), Inches(7.8), Inches(5.2),
                                size=16)
                page_num(slide, snum)

            # ══ two_column (좌우 비교) ══════════════════
            elif layout == "two_column":
                left_title   = si.get("left_title", "현황")
                left_bullets = si.get("left_bullets", [])
                right_title  = si.get("right_title", "목표")
                right_bullets= si.get("right_bullets", [])
                ...
                # 좌측 카드
                rect(slide, lx, col_y, col_w, col_h, "card")
                rect(slide, lx, col_y, Inches(0.1), col_h, "accent")
                txt(slide, left_title,
                    lx + Inches(0.2), col_y + Inches(0.15),
                    col_w - Inches(0.3), Inches(0.55),
                    size=18, bold=True, color_key="accent")
                if left_bullets:
                    add_bullets(slide, left_bullets,
                                lx + Inches(0.1), col_y + Inches(0.8),
                                col_w - Inches(0.2), Inches(4.2), size=15)

                # 우측 카드
                rect(slide, rx, col_y, col_w, col_h, "card")
                rect(slide, rx, col_y, Inches(0.1), col_h, "accent2")
                txt(slide, right_title,
                    rx + Inches(0.2), col_y + Inches(0.15),
                    col_w - Inches(0.3), Inches(0.55),
                    size=18, bold=True, color_key="accent2")
                if right_bullets:
                    add_bullets(slide, right_bullets,
                                rx + Inches(0.1), col_y + Inches(0.8),
                                col_w - Inches(0.2), Inches(4.2), size=15)
            ...
            # ══ table ══════════════════════════════════
                ...
                header_panel(slide, title_t, body, h=Inches(1.4))

                col_count = len(headers)
                row_count = len(rows) + 1
                tbl_top = Inches(1.5) if not body else Inches(1.5)
                table = slide.shapes.add_table(
                    row_count, col_count,
                    Inches(0.3), tbl_top, Inches(12.7), Inches(5.7)
                ).table
            ...
            # ══ content (기본 + body 본문 지원) ════════
            else:
                is_toc = any(str(b).startswith(("01","02","1.","2.")) for b in bullets)
                # body가 있으면 헤더 높이 늘림
                hdr_h = Inches(1.5) if body else Inches(1.3)
                header_panel(slide, title_t, body, h=hdr_h)

                content_y = hdr_h + Inches(0.1)
                content_h = H - hdr_h - Inches(0.4)
