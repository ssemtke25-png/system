...
- 단순히 행사를 설명하지 말고, 참석자들에 대한 환영의 의미와 이번 행사가 지역사회에 미치는 긍정적 영향에 대한 '철학'을 담으세요.
- 지나치게 경직된 표현보다는, 따뜻하면서도 무게감이 느껴지는 '격조 있는 구어체'를 사용하세요.
- 분량: 500자 내외.""",
        "filename": "국장인사말.docx",
    },
    "과장인사말": {
        "prompt": """다음 행사 계획서 요약을 바탕으로 과장급 인사말을 작성하세요.
- 현장 실무 책임자로서 행사를 준비한 마음가짐과 참석자들과 함께 호흡하고자 하는 의지를 담으세요.
- 친근하고 활기찬 느낌을 주되, 실무적인 안정감이 느껴지는 문체를 사용하세요.
- 분량: 400자 내외.""",
        "filename": "과장인사말.docx",
    },
    "보도자료": {
        "prompt": """다음 행사 계획서 요약을 바탕으로 언론 배포용 보도자료를 작성하세요.
- 보도자료의 생명인 '가독성'을 위해 첫 문장에 행사의 핵심 가치를 임팩트 있게 담으세요.
- "관계자는 ~라고 밝혔다"는 상투적인 문구를 최소화하고, 상황 중심의 능동적인 표현을 사용하세요.
- 마크다운 헤딩(#, ##)을 사용하세요.""",
        "filename": "보도자료.docx",
    },
}
...
{summary_str}"""
        ...
        # DOCX 저장
        paragraphs = [{"text": "현수막 문구 시안", "heading": 1}]
        for line in content.split("\n"):
            line = line.strip()
            if line:
                paragraphs.append({"text": line, "bold": line[0].isdigit()})
