...
def summarize_plan(raw_text: str) -> str:
    ...
    '신뢰감 있고 건조한 문체(행정 공문서 체계)'로 작성해라.다음은 행사 계획서 원문입니다. 아래 항목들을 JSON 형식으로 정리해주세요.
...
DOC_TYPES = {
    "요약보고서": {
        "prompt": "다음 행사 계획서 요약을 바탕으로 내부 결재용 요약보고서를 작성하세요. 마크다운 헤딩(#, ##)을 사용해 구조화하세요. 최대 A4 2장 이내로 작성하세요",
        "filename": "요약보고서.docx",
    },
    "국장인사말": {
